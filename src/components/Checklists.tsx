...
import { Machine, MachineType, Frequency, ToastMessage } from '../types';
import { api } from '../api';
...
export const Checklists: React.FC<ChecklistsProps> = ({ currentUser, addToast }) => {
  ...
  const filteredMachines = useMemo(() => {
    if (typeFilter === 'all') return allMachines;
    return allMachines.filter((m) => m.type === typeFilter);
  }, [typeFilter]);
  ...
  async function saveChecklist(machine: Machine, frequency: Frequency, items: string[]) {
    const completedIndices: number[] = [];
    items.forEach((_, idx) => {
      if (checkedItems[`${machine.id}-${frequency}-${idx}`]) {
    ...
    try {
      ...
      // Clear only this frequency's items
      const newChecked = { ...checkedItems };
      items.forEach((_, idx) => {
        delete newChecked[`${machine.id}-${frequency}-${idx}`];
      });
  ...
  function renderChecklist(machine: Machine, frequency: Frequency, items: string[]) {
    const checkedCount = items.filter((_, idx) => checkedItems[`${machine.id}-${frequency}-${idx}`]).length;
    const pct = Math.round((checkedCount / items.length) * 100);

    return (
      <div key={`${machine.id}-${frequency}`} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden mb-4">
        <div className="px-5 py-4 border-b bg-slate-50 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="w-20 h-2 bg-slate-200 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all ${pct === 100 ? 'bg-green-500' : 'bg-mac-accent'}`}
                style={{ width: `${pct}%` }}
              />
            </div>
          </div>
        ...
        <div className="divide-y divide-slate-100">
          {items.map((item, idx) => {
            const key = `${machine.id}-${frequency}-${idx}`;
        ...
        {/* Video link */}
        {machine.videos && machine.videos[frequency] && (
          <div className="px-5 py-3 border-t border-slate-100 bg-slate-50/50">
            <div className="flex items-center gap-2 text-sm text-mac-accent">
              <Icons.Play className="w-4 h-4" />
              <span className="font-medium">{machine.videos[frequency]}</span>
            </div>
          </div>
        )}
        ...
        <div className="px-5 py-3 border-t border-slate-200 bg-slate-50 flex justify-end">
          <button
            onClick={() => saveChecklist(machine, frequency, items)}
            disabled={saving}
  ...
  return (
    <div className="view-transition space-y-4">
          ...
          >
            ...
            {filteredMachines.map((m) => (
              <option key={m.id} value={m.id}>
                {m.name} — {MACHINE_TYPE_LABELS[m.type]}
              </option>
            ))}
          </select>
        </div>
          ...
          {selectedMachine.status === 'down' && (
            <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-3">
              ...
              <p className="text-sm font-bold text-red-700">
                This machine is currently marked as DOWN
              </p>
            </div>
          )}
          {(freqFilter === 'all' || freqFilter === 'weekly') &&
            selectedMachine.weekly &&
            renderChecklist(selectedMachine, 'weekly', selectedMachine.weekly)}
          {(freqFilter === 'all' || freqFilter === 'monthly') &&
            selectedMachine.monthly &&
            renderChecklist(selectedMachine, 'monthly', selectedMachine.monthly)}
        </>
      )}
      ...
      <ConfirmDialog
        ...
        message="This will uncheck all items. Are you sure?"
